    ...
    catch (Exception paramString)
    {
      while (true)
        try
        {
          ...
          continue;
          if (!CommonUtils.equalsIgnoreCase(paramString, ConfigNameEnum.LOCATE_INTERVAL.getValue()))
            continue;
          localGeoResponseInfo.setLocateInterval(CommonUtils.string2int(localXmlPullParser.nextText()));
          continue;
          if (!CommonUtils.equalsIgnoreCase(paramString, ConfigNameEnum.LOCATION_MAX_LINES.getValue()))
            continue;
          localGeoResponseInfo.setLocationMaxLines(CommonUtils.string2int(localXmlPullParser.nextText()));
          continue;
          if (!CommonUtils.equalsIgnoreCase(paramString, ConfigNameEnum.APP_INTERVAL.getValue()))
            continue;
