...
public class FindMetroFragment extends Fragment
  implements ExpandAnimation.OnExpendAnimationListener, ExpandAnimation.OnExpendActionListener, TableView.OnItemClickListener
{
  ...
  {
    paramLayoutInflater = LayoutInflater.from(getActivity()).inflate(R.layout.metro_fragment_layout, paramViewGroup, false);
    setData();
    paramViewGroup = (TableView)paramLayoutInflater.findViewById(R.id.metro_list);
  ...
  public void onItemClick(TableView paramTableView, View paramView, int paramInt, long paramLong)
  {
    ...
    if (this.mPosition == paramInt)
    {
      ...
      this.mPosition = -1;
      ((DPActivity)getActivity()).statisticsEvent("area5", "area5_moremetro", "收起", 0);
      return;
    }
    ...
    ((DPActivity)getActivity()).statisticsEvent("area5", "area5_moremetro", "展开", 0);
  }
  ...
  public void setData()
  {
    if (getActivity() != null)
    {
      this.metroList = ((RegionAndMetroActivity)getActivity()).getMetroList();
      if (this.metroList != null)
      {
        DPObject[] arrayOfDPObject = this.metroList;
  ...
  private class CategoryAdapter extends BasicAdapter
  {
    ...
    public int getCount()
    {
      return FindMetroFragment.this.parentCategory.size();
    }

    public DPObject getItem(int paramInt)
    {
      return (DPObject)FindMetroFragment.this.parentCategory.get(paramInt);
    }

    public long getItemId(int paramInt)
    {
      return ((DPObject)FindMetroFragment.this.parentCategory.get(paramInt)).getInt("ID");
    }
    ...
    public View getView(int paramInt, View paramView, ViewGroup paramViewGroup)
    {
      ...
      if (paramView == null)
        paramViewGroup = LayoutInflater.from(FindMetroFragment.this.getActivity()).inflate(R.layout.more_regionmetro_item, null);
      ...
      Object localObject3 = FindMetroFragment.this.findChildrenRegionWithLimit(((DPObject)localObject1).getInt("ID"), 3);
      paramView = (CustomGridView)paramViewGroup.findViewById(R.id.category_grid);
      ArrayList localArrayList = FindMetroFragment.this.findChildrenRegionWithLimit(((DPObject)localObject1).getInt("ID"), 2147483647);
      localObject3 = FindMetroFragment.this.formatChildrenText((ArrayList)localObject3, localArrayList.size());
      ...
      while (true)
      {
        ...
        localObject1 = new FindMetroFragment.SubCategoryAdapter(FindMetroFragment.this, localArrayList);
        ...
        paramView.setOnItemClickListener(new CustomGridView.OnItemClickListener()
        {
          ...
          {
            paramCustomGridView = new Intent("android.intent.action.VIEW", Uri.parse("dianping://regionshoplist?ismetro=true&categoryid=10&regionid=" + paramLong));
            FindMetroFragment.this.startActivity(paramCustomGridView);
            ((DPActivity)FindMetroFragment.this.getActivity()).statisticsEvent("area5", "area_moremetro_select", null, 0);
          }
        });
        ...
        paramInt = (int)Math.ceil(((FindMetroFragment.SubCategoryAdapter)localObject1).getCount() / 3.0D);
        ((LinearLayout.LayoutParams)localObject2).bottomMargin = (-(ViewUtils.dip2px(FindMetroFragment.this.getActivity(), 42.0F) * paramInt + paramInt));
 ...
 * Qualified Name:     com.dianping.main.find.FindMetroFragment
