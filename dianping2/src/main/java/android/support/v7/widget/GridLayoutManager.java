  ...
  void layoutChunk(RecyclerView.Recycler paramRecycler, RecyclerView.State paramState, LinearLayoutManager.LayoutState paramLayoutState, LinearLayoutManager.LayoutChunkResult paramLayoutChunkResult)
  {
    ...
    if (j < n)
    {
      ...
      if (paramLayoutState.mScrapList == null)
        if (bool)
        {
          ...
          label290: paramState = (LayoutParams)paramRecycler.getLayoutParams();
      ...
      while (true)
      {
        ...
        break label290;
        label422: measureChildWithDecorationsAndMargin(paramRecycler, getMainDirSpec(paramState.width), k, false);
      }
    }
    ...
    if (j < n)
    {
      ...
      while (true)
      {
        j += 1;
        break;
        label546: measureChildWithDecorationsAndMargin(paramRecycler, k, m, true);
      }
    }
    ...
    while (true)
    {
      ...
      i = i1;
      break;
      label827: i1 = getPaddingTop() + this.mCachedBorders[paramState.mSpanIndex];
      i = i1 + this.mOrientationHelper.getDecoratedMeasurementInOther(paramRecycler);
    }
    label859: Arrays.fill(this.mSet, null);
  }
